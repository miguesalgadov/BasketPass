...
import { CheckCircle, XCircle, Clock, AlertCircle, ChevronDown, Save, Users } from 'lucide-react';
...
export default function AdminAttendancePage() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        setSessions(all);
        // Default: next upcoming session; fallback to most recent past
  ...
  const loadSession = useCallback(async (sid: string) => {
    ...
    try {
      ...
      for (const rec of (aRes.data.data ?? [])) existingMap[rec.playerId] = rec.status;
      const init: Record<string, AttendanceStatus> = {};
      for (const p of playerList) init[p.id] = existingMap[p.id] ?? 'PRESENT';
      setAttendance(init);
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/attendance', {
        ...
        attendances: players.map((p) => ({ playerId: p.id, status: attendance[p.id] ?? 'PRESENT' })),
      });
  ...
  const attendanceRate = players.length > 0
    ? Math.round(((summary['PRESENT'] || 0) + (summary['LATE'] || 0)) / players.length * 100)
    : 0;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Revisá y registrá la asistencia por sesión</p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="bg-surface rounded-xl border border-border p-4">
        ...
        {loadingSessions ? (
          ...
          <p className="text-sm text-muted-foreground">No hay sesiones registradas. Creá partidos o entrenamientos en el Calendario.</p>
        ) : (
      ...
      {players.length > 0 && (
        <div className="grid grid-cols-5 gap-3">
          ...
          <div className="bg-surface rounded-xl border border-border p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 flex items-center justify-center gap-1"><Users size={10} /> Asistencia</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-surface rounded-xl border border-border overflow-hidden">
        {loadingPlayers ? (
          <div className="p-4 space-y-3">
            {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-14 bg-muted rounded-lg animate-pulse" />)}
          </div>
        ) : players.length === 0 ? (
                ...
                <div className="flex gap-1">
                  {STATUS_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button
                        key={opt.value}
                        onClick={() => setAttendance((prev) => ({ ...prev, [player.id]: opt.value }))}
                        title={opt.label}
                        className={cn(
