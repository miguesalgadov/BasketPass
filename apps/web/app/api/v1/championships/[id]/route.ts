...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const c = await prisma.championship.findFirst({
    where: { id: params.id, clubId: auth.clubId, deletedAt: null },
    include: {
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.championship.findFirst({ where: { id: params.id, clubId: auth.clubId, deletedAt: null } });
  if (!existing) return err('Championship not found', 'NOT_FOUND', 404);
